...
static float g_tileMapDef[NUM_MAP_DEFS][4]
{
    ...
    { 0.8f,0.8f,0.8f, 1.0f }, // Border
    { 0.3f,0.3f,0.3f, 1.0f }, // Floor
    { 0.8f,0.8f,0.8f, 1.0f }, // Wall
    { 1.0f,1.0f,1.0f, 1.0f }, // Pill    
    { 0.0f,1.0f,1.0f, 1.0f }, // ?
    { 0.0f,0.0f,1.0f, 1.0f }, // ?
};
...
void CSimpleTileMap::IndexToTileCoord(int a_i, int * a_tcx, int * a_tcy)
{
    *a_tcx = a_i % m_mapSize;
    *a_tcy = a_i / m_mapSize;
}

void CSimpleTileMap::WorldToTileCoord(float a_wx, float a_wy, int * a_tcx, int * a_tcy) const
{
    *a_tcx = (int)(a_wx / m_tileWidth);
    *a_tcy = (int)(a_wy / m_tileHeight);
}

void CSimpleTileMap::TileCoordToWorld(int a_tcx, int a_tcy, float * a_wx, float * a_wy, bool a_centered) const
{
    *a_wx = a_tcx * m_tileWidth;
    *a_wy = a_tcy * m_tileHeight;
    if (a_centered)
    {
        *a_wx += m_tileWidth * 0.5f;
        *a_wy += m_tileHeight * 0.5f;
    }
}

void CSimpleTileMap::GetRandomTileCoordOfValue(EMapValue a_tileValue, int* a_tcx, int* a_tcy)
{
    do
    {
        *a_tcx = rand() % m_mapSize;
        *a_tcy = rand() % m_mapSize;
    } while (GetTileMapValue(*a_tcx, *a_tcy) != a_tileValue);
}

void CSimpleTileMap::GetRandomTileCoordOfValueInRange(EMapValue a_tileValue, int a_otcx, int a_otcy, int a_range, int * a_tcx, int * a_tcy)
{
    //accessing memory in another object repetedly is probably not great. Should consider making a local variable to do the loop then pass in the result.
    do
    {
        *a_tcx = ((rand() % a_range * 2) - a_range) + a_otcx;
        *a_tcy = ((rand() % a_range * 2) - a_range) + a_otcy;
        if (*a_tcx<0)
        {
            *a_tcx = 0;
        }
