#ifndef _SIMPLEAICONTROLLER_H_
#define _SIMPLEAICONTROLLER_H_

class SimpleGameObject;
class SimpleASPathFinder;
class SimpleAIControlledGameObject;

//private because each subclass would implement their own
enum ESimpleAIStates
{
    State_Idle,
    State_Follow,
    State_Wait,
...
{
public:
    SimpleAIController(SimpleAIControlledGameObject* a_controlledGO);
    virtual ~SimpleAIController();

    void Update(float a_deltaTime);
    void Step();
    void TileReached();

    void SetTargetGO(SimpleGameObject* a_targetGO) { m_targetGO = a_targetGO; }
    SimpleGameObject* GetTargetGO() { return m_targetGO; }

    void SetPathFinder(SimpleASPathFinder* a_ASPathFinder) { m_ASPathFinder = a_ASPathFinder; }
    SimpleASPathFinder* GetPathFinder() { return m_ASPathFinder; }

    float GetSightRange() { return m_sightRange; };

protected:
    SimpleAIControlledGameObject* m_controlledGO;
    SimpleASPathFinder* m_ASPathFinder;
    SimpleGameObject* m_targetGO;
    ESimpleAIStates m_currentState;
    ...
    //Function pointer storages for state machine
    typedef void (SimpleAIController::*AIFuncPtr)();
    typedef void (SimpleAIController::*AIUpdatePtr)(float);
    AIFuncPtr m_EnterStates[ESimpleAIStates::State_LAST];
    AIUpdatePtr m_UpdateStates[ESimpleAIStates::State_LAST];
    AIFuncPtr m_StepStates[ESimpleAIStates::State_LAST];
    AIFuncPtr m_TileReachedStates[ESimpleAIStates::State_LAST];
    AIFuncPtr m_ExitStates[ESimpleAIStates::State_LAST];
    ...
    void SwitchState(ESimpleAIStates a_stateToSwitch);
};
