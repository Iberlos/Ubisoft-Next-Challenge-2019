...
{	
	...
	{	
        ...
        //---------------------------------------------------------------------------
        //Added to allow for exiting with the game pad
        bool shutdown = false;
        for (int i = 0; i < MAX_CONTROLLERS; i++)
        {
            if (App::GetController(i).IsConnected())
            {
                if (App::GetController(i).CheckButton(XINPUT_GAMEPAD_LEFT_SHOULDER, false) && App::GetController(i).CheckButton(XINPUT_GAMEPAD_RIGHT_SHOULDER, false))
                {
                    shutdown = true;
                }
            }
        ...
        //---------------------------------------------------------------------------
		if (App::IsKeyPressed(APP_QUIT_KEY) || shutdown)
		{		
			exit(0);
		}
