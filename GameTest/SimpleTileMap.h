...
enum EMapValue
{
    OUTOFBOUNDS = -1,   // If a GetTileMapValue() call is outside the bounds of the map it will return this.
    BORDER,
    FLOOR,
    WALL,
    PIP,
...
public:
    ...
    // Added Methods
    //--------------------------------------------------------------------------------------------
    void IndexToTileCoord(int a_i, int* a_tcx, int* a_tcy);
    void WorldToTileCoord(float a_wx, float a_wy, int* a_tcx, int* a_tcy) const;
    void TileCoordToWorld(int a_tcx, int a_tcy, float* a_wx, float* a_wy, bool a_centered = true) const;
    void GetRandomTileCoordOfValue(EMapValue a_tileValue, int* a_tcx, int* a_tcy);
    void GetRandomTileCoordOfValueInRange(EMapValue a_tileValue, int a_otcx, int a_otcy, int a_range, int* a_tcx, int* a_tcy);
    //--------------------------------------------------------------------------------------------
private:
    // Create a new map.
    void Create();
